    ...
    bFirst(true)
{
	...
	for(int i=0; i<numBl; i++){
		...
		Mat3x3 relativeR		= PitchBr_R.MulTM(BlRoot_R);				// Relative rotation Matrix from BladeRoot to PitchBrBottom
		//The direction of rotation around the z-axis of the right-hand system and the direction of blade pitch angle are opposite
		PitchAngle[i] = atan(-1*relativeR.dGet(1,2)/relativeR.dGet(1,1)); 
