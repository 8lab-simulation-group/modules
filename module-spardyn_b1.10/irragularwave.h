...
IrragularWave::calc_uvw(double t, const Vec3 &x)
{
    ...
    for(int i=0; i < numWave; i++)
    {
        ...
        temp += cWaves[i].zeta * cWaves[i].kappa * std::sin(cWaves[i].theta);
    }

    dzeta_dx = Vec3(temp * std::cos(waveAccelH), 
                    temp * std::sin(waveAccelH),
                            0.                  );

    zStretch = (x.dGet(3) - surfaceElev)/(waterDepth + surfaceElev) * waterDepth;

    dzs_dx = Vec3(0.,0.,1)*(waterDepth/(waterDepth + surfaceElev)) 
            - dzeta_dx*((zStretch + surfaceElev)*waterDepth/(waterDepth + surfaceElev)/(waterDepth + surfaceElev));

    //Utide = Utide0*std::pow((zStretch+TideDpth)/TideDpth , Tidepow);潮流速度を足すときは、潮流速度のヤコビアンも計算して流体速度のヤコビアンに足す

    double dVH_dzs, dVH_dth, dAH_dzs, dAH_dth, dp_dz, dp_dth;
    double dVV_dzs, dVV_dth, dAV_dzs, dAV_dth;
    for (int i = 0; i < numWave; i++)
    {
        // temporary variable
