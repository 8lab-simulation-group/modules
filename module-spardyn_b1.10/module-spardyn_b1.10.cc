...
#include "dataman.h"
#include "userelem.h"
#include "module-spardyn_b1.10.h"
...
private:
	...
	Vec3		waveV_S;
	Vec3		waveVP_S;
	doublereal  dynamicPress;

private:
	void GetWavePropertiy();
...
void
ModuleSpardyn::GetWavePropertiy() {
	if(bFirst) {
		...
		waveV = Wave.get_velocity();
		waveVP = Wave.get_acceleration();
		...
		if (bHaveSurface) {
			...
			zeta_S = Wave.get_water_elevation();
			waveV_S = Wave.get_velocity();
			waveVP_S = Wave.get_acceleration();
			dynamicPress = Wave.get_dynamic_pressure();
		}
		bFirst = false;
	}
}
...
ModuleSpardyn::CalcForce( const Vec3& vP, const Vec3& wP)
{
	...
	if(zeta > (x.dGet(3) - length/2.0) ) {
		...
		const Vec3 ur    = waveV - v;
		const Vec3 ur_N  = e3.Cross(ur.Cross(e3));
		const Vec3 uP_N  = e3.Cross(waveVP.Cross(e3));
		...
		if (bHaveSurface) {
			...
			if(zeta_S > x_S.dGet(3)) {

				const Vec3 ur_S = waveV_S - v_S;
				const Vec3 ur_S_V = e3 * ( ur.Dot(e3) );
				const Vec3 uP_S_V = e3 * ( waveVP_S.Dot(e3) );
				const Vec3 vP_S_V = e3 * ( vP_S.Dot(e3) );

				F_v = -e3 * (norm * dynamicPress * area) - vP_S_V * CavCoef + ur_S_V * ( ur_S_V.Norm() * CdvCoef); 
				F_bv = e3 * (norm *(area * waterDensity * gravity * x_S.dGet(3)));
			}
		}
		WaveForce = (F_n + F_v)*dFSF;
...
ModuleSpardyn::CalcForceJac(const Vec3& vP, const Vec3& wP)
{
	...
	if(zeta > (x.dGet(3) - length/2.0))
	{
		...
		const Mat3x3 dFni_dg = ( e3_tild * Mat3x3(MatCross, waveVP) - Mat3x3(MatCross, waveVP.Cross(e3)))* de3_dg * (CmCoef*SubCoef);
		...
		if(bHaveSurface) {
			...
			if(zeta > x_S.dGet(3)) {
				...
				Wave.calc_uvw(CurrTime,x_S);
				const doublereal dynamicPress = Wave.get_dynamic_pressure();
				...
				dFvi_dg = de3_dg * (norm*area*dynamicPress);

				// jacobian of virtical morison drag force
				const Mat3x3 urV_urVT = MultV1V2T(ur_S_V, ur_S_V);
				const doublereal urV_abs = ur_S_V.Norm();
				// d(ur_v)/dg
				const Mat3x3 dur_V_dg = MultV1V2T(e3, ur_S)*de3_dg + de3_dg * (e3.Dot(ur_S));
